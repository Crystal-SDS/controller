    ...
    if request.method == "GET":
        ...
        for it in keys:
            for key, value in r.hgetall(it).items():
                ...
                bw_limits.append({"tenant_id": it.replace("bw:AUTH_", ""), "tenant_name": tenants_list[it.replace("bw:AUTH_", "")], "policy_id": key,
...
def bw_detail(request, tenant_key):
    ...
    if request.method == "GET":
        ...
        sla = {"id": tenant_key, "tenant_id": tenant_id, "tenant_name": tenants_list[tenant_id],
               "policy_id": policy_id, "policy_name": policy_name, "bandwidth": bandwidth}
