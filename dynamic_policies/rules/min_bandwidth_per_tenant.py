    ...
    def compute_algorithm(self, info):
        ...
        for (tenant, previous_assignments) in sorted_tenants:   
            ...
            for (disk_id, transfer_speed) in previous_assignments:
                ...
                if disk_id not in computed_assignments[tenant]: computed_assignments[tenant][disk_id] = 0         
                if disk_id not in disk_usage: disk_usage[disk_id] = dict()
                if tenant not in disk_usage[disk_id]: disk_usage[disk_id][tenant] = []
        ...
        for disk_id in sorted(disk_usage):
            ...
            if disk_load > DISK_IO_BANDWIDTH:
        ...
        for disk_id in overloaded_disks.keys():
            to_redistribute = overloaded_disks[disk_id] - DISK_IO_BANDWIDTH
            ...
            for offload_tenant in tenants_to_redistribute:
                ...
                for offload_disk in computed_assignments[offload_tenant]:
                    if to_redistribute <= 0: break
                    if offload_disk == disk_id: continue
                    ...
                    if disk_load >= DISK_IO_BANDWIDTH: continue 
            ...
            if to_redistribute > 0:
                qos_disk_connections = reduce_bw_slot = 0
        ...
        free_proxy_bw = (NUM_PROXYS*PROXY_IO_BANDWIDTH)-total_bw_assigned
        if free_proxy_bw > 0: free_proxy_bw_slot = free_proxy_bw/total_disk_connections
                
        for disk_id in disk_usage.keys():
            spare_disk_capacity = DISK_IO_BANDWIDTH
            disk_connections = 0
